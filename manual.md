...
2.  **With the volume all the way down, connect your headphones or audio
    ...
    (don't worry; the port labels will be upright from this vantage).
...
5.  **When you have had enough fun for now, properly shut down
    ...
    Organelle's display will notify you when it is safe to unplug the
...
we'd do better to start with what it *can be*.
...
starting to become circular, let's cut to the chase"¦
...
someone else, and that is not just okay: it's the entire point.
...
"patch" often refers to the settings and parameter values that create
...
via Organelle's output ports. (So by analogy, these patches are closer
...
patches that are being referenced, "external" objects, etc.). Any time
...
*can* customize or create your own patches "” Pure Data is **free** and
available for all computer platforms Â "” but "to code or not to code" is
...
time. If a chapter is not important to you today, don't feel bad about
...
us, we should start with Organelle's place in the physical universe: its
hardware.

In this age of software and "virtual" devices, please do not neglect the
...
"un-computer." Configuring and then interacting with the physical
interface is just as critical as anything you do on the software side.
In fact, this is how the pretty box can become an instrument"¦
...
> "normally closed position" (negative polarity).
>
...
-   The HDMIÂ® port delivers the video output of Organelle's internal
...
-   The microSD card slot contains a card that acts as the internal
    microcomputer's root disk. We do not recommend ejecting or otherwise
    manipulating this card as nothing good will come of it.
-   The power port (labeled `9VDC`) is for connection to Organelle's own
    power supply.
...
The main face is both Organelle's primary interface with you and the
...
-   Knobs `1`, `2`, `3`, and `4` are available for parameter control
    ...
    three.\]
-   Organelle's display provides a window into its microcomputer brain,
    serving as the on-board method of monitoring and adjusting both the
    system itself and your patches.
-   The *Selector* encoder accompanies Organelle's display because they
...
> allows you to shift the the four knobs' mappings to additional (read:
> more than four) parameters. Patches with this behavior are often
> indicated by a message like *&lt;-- HOME* in the bottom line of
> Organelle's on-board display. This functionality can also be built
...
-   The maple key at the far left and its accompanying LED comprise one
    ...
    to provide the user with visual feedback of the Aux button's status.
...
-   After the Aux button, the 24 other maple keys work together as a
    ...
    are for playing notes. By default, each key triggers a "note on"
    MIDI message when it is pressed down and a corresponding "note off"
...
stereo pair to a mixer, which assumably runs to the venue's PA system,
...
port instead of the first one. The point is that Organelle's two USB
...
Organelle's own keys and knobs.

![](Ch2%20-%206%20AudioInputFromMixer+USBMIDIController.png)

By using a mixer for Organelle's audio input, we can now use multiple
audio sources with Organelle, and we can also use the mixer's various
...
Organelle's audio outputs to one of your speaker's inputs.
...
more than two devices, and an HDMI monitor for seeing Organelle's
...
microcomputer's command-line interface and graphical user interface.
...
an HDMI monitor assumes that you want to operate Organelle's
...
operating system. By learning the options provided by Organelle's
...
In this chapter, we'll explore where this combination of the Selector
...
let's start using it now and get the pretty box singing.

------------------------------------------------------------------------

## 3.1 The Selector and the On-board Display
...
After the patch is successfully loaded, Organelle's display shifts to
...
> Selector knob "”Â you don't even have to turn the Selector or call up
> the patches menu.
...
-   `Reload` rescans the `Patches` folder of the attached USB drive and
    then refreshes Organelle's patches menu. (This also unloads the
...
-   `Version` represents the version number of Organelle's operating
    system.
    ...
    -   `CPU` reports the current usage that the microcomputer's CPU is
...
Once a patch is loaded, Organelle treats this page as your "home
screen." You can return to the menu screen by turning the Selector knob,
...
-   The top line provides level meters. These are representations of the
    audio levels that are reaching the device's `I`(nputs) and those
...
> **NOTE:** The on-screen output meter is operating in a "post fader"
> mode, where the displayed signal is scaled based on the unit's current
> volume setting.
...
-   The middle four lines are prefaced with `1`, `2`, `3`, and `4`, each
    ...
    numeric representation of that parameter's current value. So in the
...
you are mainly pressing the unit's own maple keys to trigger note
...
> aren't clear to you.
>
> **NOTE:** These MIDI assignments are labeled "defaults" as you can
...
Organelle's other interface elements send out MIDI messages when they
...
"note on" messages. The leftmost key uses note number `60` ("middle C,"
or "C3" in most MIDI systems), and the rightmost key uses note number
`83` ("B4"), with all keys in between following this scheme.

When a key is pressed down, a "velocity" of `100` is transmitted. When a
key is released, a "velocity" of `0` (zero) is sent.

##### The Knobs

Knobs 1, 2, 3, and 4 transmit "control change" messages using controller
numbers `21`, `22`, `23`, and `24`, respectively. The full range of
controller values (from `0` to `127`) is utilized.

##### The Aux Button

The Aux button transmits momentary "control change" messages using
...
A pedal connected to Organelle's `Pedal` port transmits two sets of
...
you want to record the movement of Organelle's controls into a sequencer
as automation data. In other words, the mappings are a bit of a mirror.
So let's take particular note of how incoming MIDI messages interact
with and can sometimes override Organelle's on-board controls.
...
created by playing Organelle's keys. If incoming and internal notes are
occurring at the same time, these two streams are essentially merged
into one large set of notes.

##### Control Change Messages

Incoming "control change" messages using controller numbers `21`, `22`,
`23`, and `24` replace the current values set by Knobs 1, 2, 3, and 4,
respectively.

**To restore a knob's control:** simply turn the knob enough to register
a new value. In the same way that "control change" messages are designed
to work, the dominant message is always the last one received.

Incoming "control change" messages using controller number `25` affect
...
> "sustain" pedal could be quite effective.

Incoming "control change" messages using controller number `26` replace
...
Incoming "program change" messages are used to change Organelle's
current patch. "Program numbers" are assigned based on each patch's
position in alphabetical order. Let's unpack this a bit.
...
dynamically assigned a "program number" based on its position. So if
...
Using a MIDI device with Organelle is rather painless but not "hot
swappable."
...
That's about it. By remembering to reload your patch and knowing what
...
Internet will probably have some suggestions of its own"¦)
...
In this chapter, we'll look at how to work with Organelle's USB drive on
a computer. But to get there, we must begin with how that drive needs to
be configured, and we'll end with a few extra notes on USB usage with
Organelle.

With this information in hand, we'll be able to expand our patch
library. Here's to growing our set of options.
...
-   For each patch to be used by Organelle, the `Patches` folder must
    ...
    Each patch's folder must contain a file named `main.pd` that
    contains the top-level patch itself. If any other files are required
    by the patch, they must also be included in the patch's folder.
...
> "˜decompress' those files into folders before continuing. On most
> modern operating systems, this can be achieve by simply opening the
> ZIP file from the system's file browser.
...
-   **When using the included USB drive, be sure it is flipped the right
    ...
    Be sure that the exposed pins "” I think of them as teeth "”Â are
    facing "up," toward wherever the top of a connected USB cable would
...
a usable USB drive. Now that the drive is connected to a computer, let's
...
file is in place, that patch will appear in Organelle's patches menu as
...
**To add a patch to your Organelle's USB drive:** copy the properly
formatted patch folder into the USB drive's Patches folder.

**To backup a patch:** copy the patch's folder to a location on your
computer.

**To rename a patch:** rename the patch's folder, just as you would
rename any folder on your computer.

**To delete a patch:** delete the patch's folder, just as you would
...
-   **To safely unmount a connected USB disk:** from the patch
    ...
    screen's `SYSTEM` section, and select and engage the `Eject`
...
-   **USB devices are not immediately hot-swappable.** While Organelle
    ...
    reload your patch from the menu screen's patches section.
...
we have also pointed out the "un-computer" nature of this instrument.
But we also recognize that you might want run Organelle with a monitor,
keyboard, and mouse "”Â some folks might find it particularly nice to edit
patches in this fashion. So our main theme has reemerged: it's your
choice.
...
Organelle's functioning microcomputer. (As we will see very shortly,
...
Organelle. (And don't forget the USB drive housing your `Patches`
...
**To start Organelle's graphical operation mode:** type `startx`, and
then press the \[ENTER\]/\[RETURN\] key to execute the function.

> **NOTE:** Booting Organelle's graphical operation mode causes the
...
**To exit Organelle's graphical operation mode:** click the red
...
mode). This red "exit" button is the primary interface option that you
will need.

While you can navigate the file system with the keyboard and mouse, the
best way to load a patch is to do it from Organelle's hardware. By using
...
it to: the patch has been loaded, Organelle's hardware display has
...
between the Pure Data patches we run and Organelle's hardware.
...
> root directory's `mother.pd` to your `Patches` folder, you could
> experiment with editing this patch while keeping the master version
> clean. Again, you probably don't want to do this.

#### The Patch Load Sequence

To better understand the full relationship at hand, let's take a step
...
1.  If a patch is currently loaded, it receives a quitting message. This
    allows any "cleanup" processes to be executed.
...
Organelle's graphical operation mode is both expected and proper.
...
-   **To create a new patch:** duplicate a simple patch in your
    ...
    `main.pd` patch for editing. (You could also create your own "new
    patch" template for this purpose.)
-   **Explore the factory patches.** In addition to finding ideas and
    ...
    To use an external in a patch of your own, copy it to your patch's
    folder.
...
-   **Do click the red "exit" button when you are done working in this
